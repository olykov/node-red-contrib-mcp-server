...
const MAX_TAB_INDEX_NODES = 100000;
...
const TOOLS = [{
    name: 'get_flow',
    description: 'Read-only Node-RED navigation. No arguments lists tabs from the Node-RED runtime; provide a tab id to summarize one tab. Use group, chain, node, subflows or subflow for focused inspection. Results are paginated; Function code requires includeCode in node mode.',
    inputSchema: {
        type: 'object',
        properties: {
    ...
    const structuredContent = {
        ...
        meta: {
            ...
            limits: {
                ...
                tabIndexNodes: MAX_TAB_INDEX_NODES,
...
function createAdminTools({ adminPort, getAdminToken, eachNode, events, httpRequest = defaultHttpRequest }) {
    ...
    function onRuntimeEvent(event) {
        if (event && event.id === 'runtime-deploy') tabsCache = null;
    }
    ...
    function tabs() {
        ...
        let scannedNodes = 0;
        eachNode(node => {
            if (++scannedNodes > MAX_TAB_INDEX_NODES) throw new Error('Node-RED runtime exceeds tab index node limit');
        ...
        if (canSubscribe) {
            if (!subscribed) events.on('runtime-event', onRuntimeEvent);
            subscribed = true;
            tabsCache = index;
        }
