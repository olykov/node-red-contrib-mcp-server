...
const MODES = ['tabs', 'tab_summary', 'group', 'chain', 'node', 'subflows', 'subflow'];
const TOOLS = [{
    name: 'get_flow',
    description: 'Read-only Node-RED navigation. No arguments lists tabs; id alone summarizes one tab. Use group, chain, node, subflows or subflow for focused inspection. Results are paginated; Function code requires includeCode in node mode.',
    inputSchema: {
        type: 'object',
        properties: {
...
function createAdminTools({ adminPort, getAdminToken, httpRequest = defaultHttpRequest }) {
    ...
    async function tabs() {
        if (tabsCache && Date.now() < tabsCache.expires) return { ...tabsCache, cached: true };
        if (!tabsPending) {
            tabsPending = (async () => {
                const body = await read('/flows');
                const all = Array.isArray(body) ? body : body && body.flows;
                if (!Array.isArray(all)) throw new Error('Invalid Node-RED flows response');
                ...
                tabsCache = { entries, scannedNodes: all.length, expires: Date.now() + TABS_CACHE_MS };
                return { ...tabsCache, cached: false };
            })().finally(() => { tabsPending = null; });
        }
        return tabsPending;
    }
    ...
    async function callTool(toolName, args = {}) {
        ...
        if (mode === 'tabs') {
            const index = await tabs();
            const p = page(index.entries, offset);
            return result(mode, '/flows', { tabs: p.items, totalTabs: index.entries.length }, index.scannedNodes, 0, offset, p.nextOffset, { cached: index.cached });
        }
